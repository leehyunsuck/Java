...
`abstract` | 하위 클래스에서 구현 | 하위 클래스에서 오버라이드 해야함 | 인스턴스 생성이 불가능한 추상 클래스로 정의
```java
Class Food {
...
<small> 생성자의 메소드 이름은 클래스 이름과 동일 </small>
...
```
<small> `import static ...` 형식으로 입력시 클래스 이름 없이 사용 가능 </small>
